...
available — see [Dynamic mode](#dynamic-mode-legacy).

## Requirements
- [Go](https://go.dev/dl/) (build and analysis)
- Linux (4.8 or later) or macOS, on x86\_64 ("amd64") or aarch64 ("arm64")

The static analysis itself is platform-independent, but the gomodjail
binary currently builds only on Linux and macOS, as the legacy dynamic mode
is compiled in unconditionally (moving it behind a build tag is planned).
...
- Not applicable to a Go binary built by a non-trustworthy third party, as the
