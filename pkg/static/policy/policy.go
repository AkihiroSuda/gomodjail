...
//     VIOLATION and fails the gate. Dynamic mode blocked these at the
//     syscall boundary (with a few documented exceptions where the static
//     gate is deliberately stricter — see gomodjail.cm).
//   - Caveat: analysis-confidence downgrades — reflect, unsafe.Pointer,
//     assembly and //go:linkname (ARBITRARY_EXECUTION), and paths Capslock
//     could not follow. These are not proven misbehavior (dynamic mode never
