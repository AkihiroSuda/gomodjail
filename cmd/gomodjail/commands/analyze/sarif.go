...
func reportSARIF(w io.Writer, reports []policy.ModuleReport, src *profileSource, strict bool) error {
	...
	// results must serialize as [] on a clean scan, not null: SARIF defines
	// the property as an array, and code-scanning consumers can reject null.
	results := []sarifResult{}
...
func moduleLocation(src *profileSource, module string) []sarifLocation {
	...
	loc := sarifPhysicalLocation{ArtifactLocation: sarifArtifactLocation{URI: pathURI(src.GoMod)}}
...
// pathURI converts a filesystem path to a URI reference for SARIF
// artifactLocation.uri: forward slashes, percent-encoded. A relative
// --go-mod path (the common case) stays relative, which code-scanning UIs
